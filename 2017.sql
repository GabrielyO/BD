...
-- 2017
	...
	SELECT * FROM Salesman, Pieces;
	SELECT DISTINCT * FROM Salesman, Pieces;
	SELECT * FROM Salesman;
	SELECT * FROM Salesman WHERE sname BETWEEN 'Blake' AND 'Jones';
	SELECT DISTINCT * FROM Salesman s1 , Salesman s2 WHERE NOT s1.scod = s2.scod AND NOT EXISTS (SELECT DISTINCT * FROM Salesman s3 , Salesman s4 WHERE NOT s3.scod = s1.scod and s2.scod = S4.scod AND s1.scod < s4.scod) ORDER BY s1.scod ASC;
